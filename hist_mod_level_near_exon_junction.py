...
def get_df_gtf(in_args):
    ...
    if in_args.chromosome:
        out_df_gtf = out_df_gtf[out_df_gtf['chrom'] == in_args.chromosome]
    return out_df_gtf
...
def get_df_bed_mod(in_args, mod_code):
    ...
    for bedmethyl in in_args.bedmethyl:
        ...
        if in_args.chromosome:
            out_df_bed = out_df_bed[out_df_bed['chrom'] == in_args.chromosome]
...
def main():
    ...
    parser.add_argument('--mod_code', type=str, required=True, nargs=2,
                        help='Any two modification codes : ' + ', '.join(dict_display_labels.keys()))
    ...
    parser.add_argument('--chromosome', type=str, default=None,
                        help='''Only analyze data from this chromosome, else gtf and bedmethyl files 
                        are expected to be filtered such sites are within the same chromosome. 
                        Inputting cross-chromosome data might produce erroneous results.''')
